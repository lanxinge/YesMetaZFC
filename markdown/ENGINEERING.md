...
| 把句子或开放公式的推导传回裸 ZFC | [PureSentenceTransfer](../YesMetaZFC/Model/ZFC/Pure/PureSentenceTransfer.lean)、[PureOpenTransfer](../YesMetaZFC/Model/ZFC/Pure/PureOpenTransfer.lean) | 保留原模型对应和自由参数环境；无闭项时使用开放完备性 |
...
全称闭包的语义复用 [ModelClosure](../YesMetaZFC/Model/Interpretation/ModelClosure.lean) 的
`close_of_curried`，直接逐变量 `intro`，由 `Curried.apply` 统一接回任意排序的 `Values`。
参数代入后的环境等式复用 [RelationalEnvironment](../YesMetaZFC/Model/Interpretation/RelationalEnvironment.lean)
...
| 调用方需要 | 公共接口与源码 | 使用边界 |
| --- | --- | --- |
| 项与参数列的共同结构归纳 | [Logic/Syntax](../YesMetaZFC/Logic/Syntax.lean) 的 `Arguments.listRec`；[HenkinEmbedding](../YesMetaZFC/Model/Henkin/Embedding.lean)、[HenkinConservativity](../YesMetaZFC/Model/Henkin/Conservativity.lean)、[HenkinWitnessSupport](../YesMetaZFC/Model/Henkin/WitnessSupport.lean) 的相互递归和往返证明 | 保留排序、变量上下文及环境；不另造两套完整递归 |
...
Horn 图的内部反射先复用 [PureSourceHornElimination](../YesMetaZFC/Model/ZFC/Pure/PureSourceHornElimination.lean)
...
[PureSourceStrongInduction](../YesMetaZFC/Model/ZFC/Pure/PureSourceStrongInduction.lean) 的 `strong_induction` 只对具有最终阶段对应的实际正文
使用有界全称归纳；[PureSourceSyntaxRank](../YesMetaZFC/Model/ZFC/Pure/PureSourceSyntaxRank.lean) 的 `rule_ranked` 和 `rank_unique`
...
任意已检查轨迹复用 [PureSourceCheckedConstruction](../YesMetaZFC/Model/ZFC/Pure/PureSourceCheckedConstruction.lean) 的 `rule_cases_bounds`、`rule_intro_bounds`：
...
纯语言传输复用 [PureSentenceTransfer](../YesMetaZFC/Model/ZFC/Pure/PureSentenceTransfer.lean)：
...
[OpenCompleteness](../YesMetaZFC/Model/Henkin/OpenCompleteness.lean) 的 `derives_open_m`、`consistent_open_m`，
...
统一入口为 [Model](../YesMetaZFC/Model.lean)，完整接口表与使用边界见
[模型论指南](../YesMetaZFC/Model/README.md)。最小上下文代数只依赖类型化语法与异质值列，
`Model` 总入口不依赖 Henkin 完备性或具体 ZFC 模型。

- `Sem_background` 给上下文操作和成立判断，`Sem_model` 单独解释函数与关系。
  可靠性消费实际 `Sem_rules` 证书；Native 与 Ast 两个实例已核验原核六条规则。
- 原生一致性直接用 `Native.consistent`。相对标准推导用 `Sem_rules.derives_sound`，
  内部码不得从 `Proof_view` 接口自动反射为外部 `Derives`。
- 普通项保持用 `Fn_map`；上下文代数的项保持用 `Tm_map`、`Tm_substitution`。
  只在完整公式、有界见证或全部语义对象的复合定律确实需要时，使用相应更强接口。
- `Predicate_domain.Full` 只覆盖指定背景谓词域。原生完整二阶语义的同层构造及其
  实际对应为 `SecondOrder.Full.structure_of`、`predicate_full`、`satisfies_iff`。
- 复用 [Valuation](../YesMetaZFC/Model/FirstOrder/Valuation.lean) 中已推广 universe 的
  值列与环境互换；原关系消去层直接消费迁入定义，不维护平行实现。
