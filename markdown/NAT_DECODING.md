...
| 经最终纯翻译传回的内部归纳 | [PureSourceInduction](../YesMetaZFC/Model/ZFC/Pure/PureSourceInduction.lean) 的 `induction` | 性质必须是实际公式，且在内部自然数上证明原模型与规范扩张的真值一致；不授权任意外部谓词归纳 |
| 当前数码图的零／后继反演 | [PureSourceNumeralSyntax](../YesMetaZFC/Model/ZFC/Pure/PureSourceNumeralSyntax.lean) 的 `zero_iff`、`successor_iff` | 只读取内部轨迹根行，前驱证书保留同一内部集合轨迹 |
...
| 模式 2/3 共用的固定 AST 变换 | [PureSourceMappedTransform](../YesMetaZFC/Model/ZFC/Pure/PureSourceMappedTransform.lean) 中 `PureSourceInstantiation.formula_mapped_transform`；[PureSourcePointInstantiation](../YesMetaZFC/Model/ZFC/Pure/PureSourcePointInstantiation.lean) 的 `unary_point` | 变量像的合同覆盖全部 binder 深度；单槽抽象后的点实例化与自由代入共用同一个输出码 |
| 固定 AST 实例的全部语法证书 | [PureSourceInstantiationSyntax](../YesMetaZFC/Model/ZFC/Pure/PureSourceInstantiationSyntax.lean) 的 `formula_wellFormed`、`original_wellFormed` | 自由槽位的项码须在所需深度合法；数码的此项义务已由 `graph_term` 证明 |
...
| 任意内部结论码的最终阶段对应 | [PureSourceLocalTests](../YesMetaZFC/Model/ZFC/Pure/PureSourceLocalTests.lean) 的 `codeProof_agreement` | 输入证明码和结论码均属内部 ω；不要求结论是外部标准 quotation |
